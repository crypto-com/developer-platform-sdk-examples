    ...
    try:
        ...
        if ssowallet_process.poll() is None:
            ...
            if platform.system() == "Windows":
                subprocess.run(['taskkill', '/F', '/PID', str(ssowallet_process.pid)], 
        ...
        try:
            ...
            if command == "run":
                ...
                if ssowallet_process is not None and ssowallet_process.poll() is None:
                    ...
                    confirm = input("Do you want to stop it before exiting? (y/n): ").strip().lower()
                    if confirm == 'y':
                        stop_ssowallet()
            ...
            if ssowallet_process is not None and ssowallet_process.poll() is None:
                confirm = input("Do you want to stop the running SSO Wallet agent? (y/n): ").strip().lower()
                if confirm == 'y':
                    stop_ssowallet()
            print("Exiting control script.")
            break
